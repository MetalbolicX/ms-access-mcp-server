// normalize.mjs — shared normalizer for the differential parity harness.
//
 ...
 */
export function normalize(value, volatileFields = []) {
  ...
  if (typeof value === "object") {
    ...
    for (const k of keys) {
      ...
      out[k] = normalize(value[k], volatileFields);
    }
    return out;
  }
  return value;
...
export function diff(expected, actual, path = "$") {
  ...
  if (typeof expected === "object") {
    ...
    for (const k of ek) {
      const r = diff(expected[k], actual[k], `${path}.${k}`);
      if (r !== null) return r;
    }
    return null;
  }
