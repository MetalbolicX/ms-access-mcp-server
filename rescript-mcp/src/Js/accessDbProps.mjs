...
// ---------------------------------------------------------------------------
export const getDatabaseProperties = (app, names) => {
    return new Promise((resolve) => {
        try {
                ...
                if (dbObj.Properties) {
                    ...
                    for (let i = 0; i < count; i++) {
                        try {
                            ...
                            if (!name)
                                continue;
                            if (name.startsWith("_") || name.startsWith("MSys"))
                                continue;
                            if (namesFilter && !namesFilter.has(name.toLowerCase()))
                                continue;
                            let val = prop.Value;
                            if (val === null)
                                val = "";
            ...
            // Project info
            try {
                const cp = accessApp.CurrentProject;
                if (cp) {
...
// ---------------------------------------------------------------------------
export const setDatabaseProperty = (app, name, value, type_) => {
    return new Promise((resolve) => {
        try {
            const accessApp = app;
            const db = accessApp.CurrentDb ? accessApp.CurrentDb() : null;
            ...
            let coerced;
            if (daoType === 1)
                coerced = ["true", "1", "yes", "-1"].includes(value.toLowerCase());
            else if (daoType === 2 || daoType === 3 || daoType === 4)
                coerced = parseInt(value) || 0;
            else if (daoType === 7)
                coerced = parseFloat(value) || 0.0;
            else
                coerced = value;
...
// ---------------------------------------------------------------------------
export const exportModuleToText = (app, moduleName) => {
    return new Promise((resolve) => {
        try {
            const accessApp = app;
...
// ---------------------------------------------------------------------------
export const _tempSaveAsText = (app, objectType, objectName) => {
    return new Promise((resolve) => {
        const tmp = pathJoin(tmpdir(), "mcp_exp_" + randomBytes(8).toString("hex") + ".txt");
        try {
            const accessApp = app;
...
// ---------------------------------------------------------------------------
export const _tempLoadFromText = (app, objectType, objectName, textData) => {
    return new Promise((resolve) => {
        const tmp = pathJoin(tmpdir(), "mcp_imp_" + randomBytes(8).toString("hex") + ".txt");
        try {
