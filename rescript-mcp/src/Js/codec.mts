...
const CP1252_ENCODE: Map<number, number> = new Map([
...
export const encodeCp1252 = (text: string): number[] => {
  const result: number[] = []
...
export const decodeCp1252 = (bytes: number[]): string => {
...
export const bufferToBytes = (buf: Buffer): number[] => Array.from(buf)
...
export const decodeUtf16LeSkipBom = (bytes: number[]): string => {
